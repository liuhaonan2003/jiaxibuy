...
class Tree extends Think {
	...
	function get_head_nav($bid,$maxlevel,$effected_id='navlist',$style='filetree ' ,$homefont='',$recursion=FALSE ,$child='',$showenname='',$site_url='') {
		if($showenname) $indexen =  '<em>home</em>';
		if($homefont) $homefont='<li><a href="'.$site_url.'">'.$homefont.$indexen.'</a></li>';
		$number=1;
		if(!$child) $child = $this->getchild($bid);
